...
import surfai
from surfai import error, util, version
from surfai.openai_response import OpenAIResponse
from surfai.util import ApiType
...
def _requests_proxies_arg(proxy) -> Optional[Dict[str, str]]:
    ...
    if proxy is None:
        ...
        raise ValueError(
            "'surfai.proxy' must be specified as either a string URL or a dict with string URL under the https and/or http keys."
        )
...
def _make_session() -> requests.Session:
    if surfai.requestssession:
        if isinstance(surfai.requestssession, requests.Session):
            return surfai.requestssession
        return surfai.requestssession()
    if not surfai.verify_ssl_certs:
        warnings.warn("verify_ssl_certs is ignored; surfai always verifies.")
    s = requests.Session()
    proxies = _requests_proxies_arg(surfai.proxy)
    if proxies:
        s.proxies = proxies
    s.mount(
        "https://",
...
class APIRequestor:
    def __init__(
        ...
        self.api_base = api_base or surfai.api_base
        self.api_key = key or util.default_api_key()
        self.api_type = (
            ApiType.from_str(api_type)
            if api_type
            else ApiType.from_str(surfai.api_type)
        )
        self.api_version = api_version or surfai.api_version
        self.organization = organization or surfai.organization
    ...
    def request_headers(
        ...
        if surfai.app_info:
            user_agent += " " + self.format_app_info(surfai.app_info)
        ...
        if surfai.app_info:
            ua["application"] = surfai.app_info
    ...
    def _prepare_request_raw(
        ...
        if method == "get" or method == "delete":
            ...
            raise error.APIConnectionError(
                ...
                "OpenAI bindings. Please contact us through our help center at help.surfai.com for "
                "assistance." % (method,)
            )
    ...
    async def arequest_raw(
        ...
        *,
        ...
        request_kwargs = {
            ...
            "headers": headers,
            "data": data,
            "proxy": _aiohttp_proxies_arg(surfai.proxy),
            "timeout": timeout,
        }
        try:
...
async def aiohttp_session() -> AsyncIterator[aiohttp.ClientSession]:
    user_set_session = surfai.aiosession.get()
    if user_set_session:
