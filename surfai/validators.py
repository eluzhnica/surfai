...
from surfai.datalib.pandas_helper import assert_has_pandas
...
def write_out_file(df, fname, any_remediations, auto_accept):
    ...
    if not any_remediations and not split:
        ...
        sys.stdout.write(
            f'\nWrote modified file{files_string}`\nFeel free to take a look!\n\nNow use that file when fine-tuning:\n> surfai api fine_tunes.create -t "{fnames[0]}"{valid_string}{additional_params}\n\n{separator_reminder}{optional_ending_string}\n'
        )
        estimate_fine_tuning_time(df)
    else:
