...
def get_object_classes():
    ...
    from surfai.object_classes import OBJECT_CLASSES
...
def convert_to_openai_object(
    ...
    if isinstance(resp, surfai.openai_response.OpenAIResponse):
    ...
    if plain_old_data:
        ...
        resp, surfai.openai_object.OpenAIObject
    ):
        resp = resp.copy()
        klass_name = resp.get("object")
        if isinstance(klass_name, str):
            klass = get_object_classes().get(
                klass_name, surfai.openai_object.OpenAIObject
            )
        else:
            klass = surfai.openai_object.OpenAIObject
...
def default_api_key() -> str:
    if surfai.api_key_path:
        with open(surfai.api_key_path, "rt") as k:
            api_key = k.read().strip()
            if not api_key.startswith("sk-"):
                raise ValueError(f"Malformed API key in {surfai.api_key_path}.")
            return api_key
    elif surfai.api_key is not None:
        return surfai.api_key
    else:
        raise surfai.error.AuthenticationError(
            "No API key provided. You can set your API key in code using 'surfai.api_key = <API-KEY>', or you can set the environment variable OPENAI_API_KEY=<API-KEY>). If your API key is stored in a file, you can point the surfai module at it with 'surfai.api_key_path = <PATH>'. You can generate API keys in the OpenAI web interface. See https://platform.openai.com/account/api-keys for details."
        )
