...
import surfai
from surfai.datalib.numpy_helper import numpy as np
from surfai.datalib.pandas_helper import pandas as pd
...
def get_embedding(text: str, engine="text-similarity-davinci-001", **kwargs) -> List[float]:
    ...
    return surfai.Embedding.create(input=[text], engine=engine, **kwargs)["data"][0]["embedding"]
...
async def aget_embedding(
    ...
    return (await surfai.Embedding.acreate(input=[text], engine=engine, **kwargs))["data"][0][
        "embedding"
    ]
...
def get_embeddings(
    ...
    data = surfai.Embedding.create(input=list_of_text, engine=engine, **kwargs).data
...
async def aget_embeddings(
    ...
    data = (await surfai.Embedding.acreate(input=list_of_text, engine=engine, **kwargs)).data
    return [d["embedding"] for d in data]
