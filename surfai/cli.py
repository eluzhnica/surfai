...
import surfai
from surfai.upload_progress import BufferReader
from surfai.validators import (
...
class Engine:
    @classmethod
    def get(cls, args):
        engine = surfai.Engine.retrieve(id=args.id)
        display(engine)

    @classmethod
    def update(cls, args):
        engine = surfai.Engine.modify(args.id, replicas=args.replicas)
        display(engine)
    ...
    def generate(cls, args):
        ...
        if args.completions and args.completions > 1 and args.stream:
            raise ValueError("Can't stream multiple completions with surfai CLI")

        kwargs = {}
        if args.model is not None:
            kwargs["model"] = args.model
        resp = surfai.Engine(id=args.id).generate(
...
class ChatCompletion:
    ...
    def create(cls, args):
        ...
        resp = surfai.ChatCompletion.create(
            # Required
            model=args.model,
            engine=args.engine,
            messages=messages,
...
class Deployment:
    @classmethod
    def get(cls, args):
        resp = surfai.Deployment.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = surfai.Deployment.delete(args.id)
        print(model)

    @classmethod
    def list(cls, args):
        models = surfai.Deployment.list()
        print(models)

    @classmethod
    def create(cls, args):
        models = surfai.Deployment.create(
...
class Model:
    @classmethod
    def get(cls, args):
        resp = surfai.Model.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = surfai.Model.delete(args.id)
        print(model)
...
class File:
    ...
    @classmethod
    def get(cls, args):
        resp = surfai.File.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        file = surfai.File.delete(args.id)
        print(file)

    @classmethod
    def list(cls, args):
        file = surfai.File.list()
        print(file)
...
class Image:
    ...
    def create_variation(cls, args):
        with open(args.image, "rb") as file_reader:
            buffer_reader = BufferReader(file_reader.read(), desc="Upload progress")
        resp = surfai.Image.create_variation(
            image=buffer_reader,
            size=args.size,
            n=args.num_images,
            response_format=args.response_format,
...
class Audio:
    ...
    def transcribe(cls, args):
        ...
        resp = surfai.Audio.transcribe_raw(
            # Required
            model=args.model,
            file=file_reader,
            filename=args.file,
    ...
    def translate(cls, args):
        with open(args.file, "rb") as r:
            file_reader = BufferReader(r.read(), desc="Upload progress")
        resp = surfai.Audio.translate_raw(
            # Required
            model=args.model,
            file=file_reader,
            filename=args.file,
...
class FineTune:
    ...
    def _maybe_upload_file(
        ...
        if check_if_file_exists:
            bytes = len(content)
            matching_files = surfai.File.find_matching_files(
    ...
    def _get_or_upload(cls, file, check_if_file_exists=True):
        try:
            ...
            surfai.File.retrieve(file)
            return file
        except surfai.error.InvalidRequestError:
            pass
    ...
    def create(cls, args):
        ...
        resp = surfai.FineTune.create(**create_args)
    ...
    @classmethod
    def get(cls, args):
        resp = surfai.FineTune.retrieve(id=args.id)
        print(resp)

    @classmethod
    def results(cls, args):
        fine_tune = surfai.FineTune.retrieve(id=args.id)
        if "result_files" not in fine_tune or len(fine_tune["result_files"]) == 0:
            raise surfai.error.InvalidRequestError(
                f"No results file available for fine-tune {args.id}", "id"
            )
        result_file = surfai.FineTune.retrieve(id=args.id)["result_files"][0]
        resp = surfai.File.download(id=result_file["id"])
        print(resp.decode("utf-8"))

    @classmethod
    def events(cls, args):
        if args.stream:
            raise surfai.error.OpenAIError(
                message=(
                    "The --stream parameter is deprecated, use fine_tunes.follow "
                    "instead:\n\n"
                    "  surfai api fine_tunes.follow -i {id}\n".format(id=args.id)
                ),
            )

        resp = surfai.FineTune.list_events(id=args.id)  # type: ignore
        print(resp)
    ...
    def _stream_events(cls, job_id):
        def signal_handler(sig, frame):
            status = surfai.FineTune.retrieve(job_id).status
            sys.stdout.write(
                "\nStream interrupted. Job is still {status}.\n"
                "To resume the stream, run:\n\n"
                "  surfai api fine_tunes.follow -i {job_id}\n\n"
                "To cancel your job, run:\n\n"
                "  surfai api fine_tunes.cancel -i {job_id}\n\n".format(
        ...
        events = surfai.FineTune.stream_events(job_id)
        ...
        resp = surfai.FineTune.retrieve(id=job_id)
        status = resp["status"]
        if status == "succeeded":
            sys.stdout.write("\nJob complete! Status: succeeded 🎉")
            sys.stdout.write(
                "\nTry out your fine-tuned model:\n\n"
                "surfai api completions.create -m {model} -p <YOUR_PROMPT>".format(
                    model=resp["fine_tuned_model"]
                )
            )
        elif status == "failed":
            sys.stdout.write(
                "\nJob failed. Please contact us through our help center at help.surfai.com if you need assistance."
            )
        sys.stdout.write("\n")

    @classmethod
    def cancel(cls, args):
        resp = surfai.FineTune.cancel(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        resp = surfai.FineTune.delete(sid=args.id)
        print(resp)
...
class WandbLogger:
    @classmethod
    def sync(cls, args):
        import surfai.wandb_logger

        resp = surfai.wandb_logger.WandbLogger.sync(
            id=args.id,
...
Mutually exclusive with `top_p`.""",
    ...
    sub.add_argument(
        ...
        'For example, `surfai api fine_tunes.create -t test.jsonl -m ada --suffix "custom model name" '
